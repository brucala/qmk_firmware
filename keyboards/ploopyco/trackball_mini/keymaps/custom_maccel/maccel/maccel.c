...
// for a description of these parameter choices see:
// https://github.com/burkfers/qmk_userspace_features/issues/11
...
#    define MACCEL_GROWTH_RATE 0.65 // lower/higher value = curve reaches its upper limit slower/faster
#endif
#ifndef MACCEL_OFFSET
#    define MACCEL_OFFSET 1.5 // lower/higher value = acceleration kicks in earlier/later
#endif
#ifndef MACCEL_LIMIT
#    define MACCEL_LIMIT 0.12 // lower limit of accel curve (minimum acceleration factor)
#endif
...
#    define MACCEL_LIMIT_UPPER 2 // upper limit of accel curve, recommended to leave at 1; adjust DPI setting instead.
#endif
...
// console output for debugging (enable/disable in config.h)
    ...
    uprintf(
        //"MACCEL: DPI:%d Tko: %d Grw: %d Ofs: %d Lmt: %d | Fct: %d v.in: %d v.out: %d d.in: %d d.out: %d\n",
        "Fct: %d v.in: %d d.in: %d\n",// d.out: %d\n",
        //(int)(100 * device_cpi),
        //(int)(100 * g_maccel_config.takeoff),
        //(int)(100 * g_maccel_config.growth_rate),
        //(int)(100 * g_maccel_config.offset),
        //(int)(100 * g_maccel_config.limit),
        (int)(100 * maccel_factor),
        (int)(100 * velocity),
        //(int)(100 * velocity * maccel_factor),
        CONSTRAIN_REPORT(distance)
        //CONSTRAIN_REPORT(100*sqrtf(x * x + y * y))
    );
#endif // MACCEL_DEBUG
